...
Node and NPM are required. Installing the latest lts is advised. [Node Download](https://nodejs.org/en/download)
...
## Running
Copy the resources folder from `ssl-server-student/src/main/resources` to the repos `backend/src/main` directory. This moves the keystore and properties file to the spring-boot project to enable ssl and setup the server on port 8443. 

If both the assignment and repo are in the same directory, this command should work from the cloned repos root.
